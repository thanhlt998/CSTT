...
class FuzzyClassifier:
    ...
    def get_combined_rules(self):
        r = {}
        for r1 in [method for method in dir(Height) if callable(getattr(Height, method) and method.startswith('fuzzy'))]:
            for r2 in [method for method in dir(Weight) if callable(getattr(Weight, method) and method.startswith('fuzzy'))]:
                r["%s|%s" %(r1.__name__, r2.__name__)] = FuzzyAttribute()
        self.deleted_rule = []
    ...
    def calculate_beta(self):
        for key, value in self.combined_rules:
    ...
    def deduce(self, cls_attr):
        cls_attr = [(k, cls_attr[k]) for k, v in sorted(cls_attr, key=v.get_alpha())]
        if len(cls_attr) == 1:
